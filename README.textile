...
To install administrate_me plugin into your rails project (rails version >= 2.1) just run:

<pre><code>
  ruby script/plugin install git://github.com/insignia/administrate_me.git 
</code></pre>

For projects running with rails < 2.1, you'll have to clone plugin's git repository and delete .git folder  after it, just like this:
...
* *So, what are waiting for!? Run the application!*
  ...
  <%=
    present :elegant do |p|
      ...
      p.body do |r|
        ...
        r.field 'Created at', @resource.created_atrequire File.expand_path(File.dirname(__FILE__) + '/../../spec_helper')

describe Box::HitsController do

  before(:each) do
    @mock_model_class = Hit
    @user = mock_model(User)
    @user.stub!(:id).and_return(1)
    controller.stub!(:current_user).and_return(@user)
    controller.stub!(:before_save).and_return(true)
  end
...
administrate_me put together by people at "(in)signia":http://www.insignia4u.com/
